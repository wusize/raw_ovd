...
sys.path.extend(["/mnt/cache/wusize/raw_ovd", ])
...
output_root = "output/lvis_samples"
original_images = "output/lvis_rare_images"
os.makedirs(output_root, exist_ok=True)
image_root = 'datasets/coco/train2017'
json_path = 'datasets/lvis/lvis_v1_train_rare_anns_and_imgs.json'
lvis = COCO(json_path)
proposal_path = \
    'output/Detic/base_res50_fpn_lvis_20e_get_train_rare_proposals/' \
    'inference_lvis_v1_train_rare/instances_predictions.pth'
instances = torch.load(proposal_path)
img2instances = {inst['image_id']: inst['proposals'] for inst in instances}
novel_cat_ids = [cat_id for cat_id, cat in lvis.cats.items() if cat['frequency'] == 'r']
...
for img_id, img_info in tqdm(lvis.imgs.items()):
    ...
    cv2.imwrite(os.path.join(original_images, file_name), image)
    image_shape = image.shape[:2]
    instances = img2instances[img_id]
    img2ann = lvis.imgToAnns[img_id]
    annos = []
    is_novel = []
    for obj in img2ann:
