...
class GradCAM():
    ...
    def _forward_backward_pass(self, inputs, target_instance):
        ...
        if target_instance == None:
            ...
            # assert len(output[
            #                'instances']) >= target_instance, f"Only {len(output['instances'])} objects found but you request object number {target_instance}"
            match = output['instances'].pred_classes == target_instance
            scores = output['instances'].scores[match]
            if len(scores) > 0:
                score = scores.max()
            else:
                target_instance = np.argmax(output['instances'].scores.cpu().data.numpy(), axis=-1)
