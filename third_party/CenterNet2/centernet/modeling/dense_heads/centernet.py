...
class CenterNet(nn.Module):
    ...
    def forward(self, images, features_dict, gt_instances,
        ...
        if self.training and return_loss:
            pos_inds, labels, reg_targets, flattened_hms, hm_weights = \
                self._get_ground_truth(
                    grids, shapes_per_level, gt_instances, ann_types)
            ...
            # TODO: adjust logits_pred and agn_hm_pred
            losses = self.losses(
                pos_inds, labels, reg_targets, flattened_hms,
                logits_pred, reg_pred, agn_hm_pred, hm_weights)
    ...
    def losses(
        ...
        '''
        # pre_process
        hm_weights = hm_weights.view(-1)
        min_logit = inverse_sigmoid(torch.tensor(0.0), self.sigmoid_clamp).item()
        if logits_pred is not None:
            logits_pred[hm_weights < 1.0] = min_logit
        flattened_hms[hm_weights < 1.0] = 0.0
        if agn_hm_pred is not None:
            agn_hm_pred[hm_weights < 1.0] = min_logit
        del hm_weights
    ...
    def _get_ground_truth(self, grids, shapes_per_level, gt_instances, ann_types):
        '''
        Input:
            grids: list of tensors [(hl x wl, 2)]_l
            shapes_per_level: list of tuples L x 2:
            gt_instances: gt instances
            ann_types: with_instance or not
        Retuen:
            pos_inds: N
            labels: N
            reg_targets: M x 4
        ...
        for i in range(len(gt_instances)): # images
            ...
            if N == 0:
                ...
                flattened_hm = grids.new_zeros((
                    M, 1 if self.only_proposal else heatmap_channels))
                flattened_hms.append(flattened_hm)
                hm_weight = torch.ones_like(flattened_hm[..., :1]) \
                    if ann_types[i] == 'with_instance' else torch.zeros_like(flattened_hm[..., :1])
                hm_weights.append(hm_weight)
                continue
            ...
            # TODO delete this when importing image-level annotations
            assert ann_types[i] == 'with_instance'
        ...
        hm_weights = _transpose(hm_weights, num_loc_list)
        ...
        hm_weights = cat([x for x in hm_weights], dim=0)  # MB x C
        assert flattened_hms.shape[0] == hm_weights.shape[0], \
            f"flattened_hms: {flattened_hms.shap}, hm_weights: {hm_weights.shape}"
        
        return pos_inds, labels, reg_targets, flattened_hms, hm_weights
