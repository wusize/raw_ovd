...
class ContextModellingV5(ContextModelling):
    def _get_clip_word_features(self, pseudo_words, word_mask, clip_model, positional_encoder):
        pseudo_positions = torch.tensor([[0.5, 0.5, 1.0, 1.0]]).to(pseudo_words.device)
        pseudo_words = pseudo_words + positional_encoder(pseudo_positions)   # add pseudo positions
        word_mask = word_mask.half()
        start_end_mask = torch.ones_like(word_mask[:, :1])
        valid_mask = torch.cat([start_end_mask, word_mask, start_end_mask], dim=-1)   # add end_start mask
    ...
    def kd_clip_contrast(self,
        ...
        # TODO: get per-pred clip_word_feature
        if self.checkboard_cfg.LOCAL_CORRESPONDENCE:
            clip_word_features = self._get_clip_word_features(pseudo_words, word_masks,
                                                              clip_model, positional_encoder)
        else:
