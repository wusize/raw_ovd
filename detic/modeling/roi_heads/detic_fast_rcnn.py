...
class DeticFastRCNNOutputLayers(FastRCNNOutputLayers):
    ...
    def pred_cls_score(self, pseudo_words):
        ...
        with autocast():
            if self.word_dropout > 0.0 and self.training:
                attn_mask, pe_indices = self._drop_word(pseudo_words.half(), self.clip.transformer.heads)
            else:
                attn_mask, pe_indices = None, None
            pseudo_text, end_token_ids = self.clip.prepare_pseudo_text_tensor(
                pseudo_words.half())  # add start and stop token
            # assert attn_mask.shape[:2] == pseudo_words.shape[:2]
            cls_features, x, end_token_ids = \
                self.clip.encode_pseudo_text_endk(pseudo_text, end_token_ids,
                                                  text_pe=True,
