...
from .detic_fast_rcnn import MultiLevelFastRCNNOutputLayers
...
class FPNStandardROIHeads(StandardROIHeads):
    @configurable
    def __init__(self, **kwargs):
        cfg = kwargs.pop('cfg')
        super(FPNStandardROIHeads, self).__init__(**kwargs)
        ...
        self.context_modeling = ContextModelling(self.context_modeling_cfg,
                                                 num_words=num_levels * self.box_predictor.num_words,
    ...
    def _box_forward_train(self, box_features, proposals):
        ...
        num_levels = len(self.cfg.MODEL.ROI_HEADS.IN_FEATURES)
        input_box_features = input_box_features.view(pseudo_words.shape[0], num_levels, -1)
        proposal_deltas = self.box_predictor.pred_box(input_box_features[sample_types == 0])
    ...
    def _init_box_head(cls, cfg, input_shape):
        ...
        box_predictor = MultiLevelFastRCNNOutputLayers(
...
class AllLevelROIPooler(ROIPooler):
    ...
    def forward(self, x, box_lists):
        ...
        output = torch.stack(output, dim=1)     # num_boxes, num_levels, ...
