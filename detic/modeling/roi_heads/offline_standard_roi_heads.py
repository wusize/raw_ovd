...
from detic.modeling.roi_heads.offline_context_modelling \
    import OfflineContextModelling as ContextModelling
from time import time
from detectron2.modeling.poolers import convert_boxes_to_pooler_format


@ROI_HEADS_REGISTRY.register()
class OfflineStandardROIHeads(StandardROIHeads):
    @configurable
    ...
    def forward(self, images, features, proposals, targets=None,
        ...
        if self.training:
            proposals, group_infos = self.label_and_sample_proposals(
                proposals, targets, ann_types=ann_types, image_ids=image_info.keys())
            del targets
    ...
    def label_and_sample_proposals(self, proposals, targets, ann_types, image_ids):
        ...
        num_bg_samples = []
        group_infos = []
        for proposals_per_image, targets_per_image, ann_type, image_id in zip(
                proposals, targets, ann_types, image_ids):
            ...
            added_instances, group_info = self.context_modeling.sample(targets_per_image,
                                                                       image_id, self.mask_on)
