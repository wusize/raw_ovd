...
class ContextModellingFocal(ContextModelling):
    def kd_clip_contrast(self,
        ...
        loss = 0.5 * self.focal_loss(similarity_matrix_0, label) \
               + 0.5 * self.focal_loss(similarity_matrix_1, label)
        ...
        if self.checkboard_cfg.LOCAL_CORRESPONDENCE:
            ...
            loss = self.focal_loss(similarity_matrix_0, labels) * 0.5 \
                   + self.focal_loss(similarity_matrix_1, labels) * 0.5
    ...
    def focal_loss(self, x, target):
        return softmax_focal_loss(x, target, gamma=self.cfg.FOCAL_GAMMA)


def softmax_focal_loss(x, target, gamma=2.):
    p = torch.softmax(x, dim=1)
    p = p.gather(dim=1, index=target[:, None]).view(-1)
    assert len(p) == len(target)
    weights = (1 - p) ** gamma     # do not detach the weights

    loss = -weights * torch.log(p)
    loss = loss.mean()
